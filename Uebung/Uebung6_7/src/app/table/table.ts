...
export class Table implements OnInit{
  ...
  async ngOnInit(): Promise<void> {
    const data = await this.memberservice.getAllMembers() //einmal service aufrufen, um alle Members zu holen -> da teuer!
  ...
  filter() {
    ...
    const value = this.search.value?.toLowerCase() || ''; //solle value null sein, dann nehmen wir den leeren string
